...
def test_api_session_get_uses_timeout(mocker: MockerFixture) -> None:
    ...
    response = api_session.get("https://example.com/api")

    assert response == mock_response
    session_instance.get.assert_called_once_with("https://example.com/api", timeout=15)
...
def test_api_session_get_overrides_timeout(mocker: MockerFixture) -> None:
    ...
    api_session.get("https://example.com/api", timeout=5)

    session_instance.get.assert_called_once_with("https://example.com/api", timeout=5)
